...
// a table with ACK's from differenrt users
// (ipaddr,port) -> ACK
map<pair<string, int>, bool> ACK_Table;

// nickname -> [(x0,y0),(x1,y1),...,(x6,y6)]
void sleepcp(int milli, bool &ACK)
{
...
int receiveMsg(string msg, string ipaddr, int port)
{
    ...
    inet_aton(ipaddr.c_str(), &cliaddr.sin_addr);
    cliaddr.sin_port = port;

    int n;
    char buffer[MAXLINE];
    unsigned int len = sizeof(cliaddr);
    pair<string, int> info_user = make_pair(ipaddr, port);

    // ACK (3B)
    // nickname (1B)
    if (msg == "ACK")
    {
        ACK_Table[info_user] = 1;
    }
    else
    {
        if (msg[0] == 'N')
        {
            ACK_Table[info_user] = 0;
            bool &ACK = ACK_Table[info_user];
            if (tableUsers.empty()) // the users table is empty so, we add this nickname
            {
                ...
                do
                {
                    sendto(sockfd, "O200", MAXLINE,
                           ...
                           len);
                    sleepcp(300, ACK); // 300 milliseconds for waiting
                } while (!ACK);
...
int main()
{
    ...
    while (1)
    {
        ...
        buffer[n] = '\0';
        msg.assign(buffer);
        // reenviamos el ack al cliente que lo envió
